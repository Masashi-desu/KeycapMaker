...
- `深さ` は 0 以上、かつキートップの最高点が下がらない最大値まで入力でき、0 へ戻すとフラット相当へ戻る
- `キートップ形状 = シンドリカル / スフェリカル` の `深さ` を浅くしても、凹みの開始位置が中心へ寄らず、外周基準のまま深さだけ変わる
- `キートップ形状 = シンドリカル / 深さ = 1.4`、`キートップ形状 = スフェリカル / 深さ = 1.45` のような過大入力は上限へ丸められ、body / legend / homing bar が描画される
