...
custom shell と JIS Enter の `keycapEdgeRadius` は、キートップ上面とサイドウォールの境目だけに適用する R 面取りとして扱う。0 は従来の角面、正値は既存の shoulder 生成を維持したうえで上端の局所的な roundover だけを追加する。球状に近いキートップ全体の盛り上げはこのパラメータの責務に含めず、将来的に spherical top のマイナス `dishDepth` 側で調整する。
