...
キートップ形状は `topSurfaceShape` で `flat` / `cylindrical` / `spherical` を切り替える。`dishDepth` は 0 以上の値だけを受け、正値で凹み量を表す。入力上限は現在の `dishRadius` とキートップ上面 footprint から、dish を深くしてもキートップの最高点が下がらない範囲へ丸める。盛り上がり形状は `dishDepth` の負値では扱わず、別パラメータとして設計する。
