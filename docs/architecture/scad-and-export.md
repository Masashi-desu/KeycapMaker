...
top legend の曲面追従 volume は、上面曲面とその下側へ平行移動した曲面の間の band として扱う。深い cylindrical / spherical dish でも、平面基準の埋め込み深さだけで切り出して legend part が空になることを避ける。
...
キートップ形状は `topSurfaceShape` で `flat` / `cylindrical` / `spherical` を切り替える。`dishDepth` は 0 以上の値だけを受け、正値で凹み量を表す。入力上限は現在の `dishRadius` とキートップ上面 footprint から、dish を深くしてもキートップの最高点が下がらない範囲へ丸める。凹みの開始位置は上面 footprint 基準で固定し、深さ変更時は球 / 円柱の Z 方向だけを正規化して中心の落ち込み量を変える。盛り上がり形状は `dishDepth` の負値では扱わず、別パラメータとして設計する。
