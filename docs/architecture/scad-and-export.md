...
custom shell は `topHatEnabled` で上面にもう一つの小さいキートップを追加できる。top-hat は body 側の形状として扱い、`topHatTopWidth` / `topHatTopDepth` / `topHatTopRadius` / `topHatHeight` / `topHatShoulderAngle` を `user_*` へ渡す。`topHatHeight` がマイナスの場合は同じ形状を上面から凹ませ、シェル天井を貫通しない深さに丸める。JIS Enter や typewriter 系にはまだ表示しない。
