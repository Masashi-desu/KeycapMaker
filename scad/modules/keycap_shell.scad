...
function keycap_inner_height(top_center_height, dish_depth, top_thickness) =
    max(top_center_height - max(dish_depth, 0) - top_thickness, 0.2);
...
function keycap_top_edge_effective_radius(
    radius,
    top_center_height,
    shoulder_outset,
    top_width,
...
function keycap_top_edge_surface_inset(
    radius,
    top_center_height,
    shoulder_outset,
    top_width,
    ...
    keycap_top_edge_top_inset(
        keycap_top_edge_effective_radius(
            radius,
            top_center_height,
            shoulder_outset,
            top_width,
...
function keycap_shell_shoulder_curve_amount(height, shoulder_outset, shoulder_radius) =
    let(
        safe_shoulder_radius = abs(keycap_top_hat_safe_shoulder_radius(shoulder_radius)),
        shoulder_radius_limit = max(min(height, shoulder_outset), 0)
    )
    shoulder_radius_limit <= 0.001
        ? 0
        : min(safe_shoulder_radius / shoulder_radius_limit, 1) * (shoulder_radius < 0 ? -1 : 1);

function keycap_shell_shoulder_plan_fraction_for_z(
    z_fraction,
    ...
    let(
        target = min(max(z_fraction, 0), 1),
        mid = (low + high) / 2,
        mid_z = keycap_shell_shoulder_z_fraction(mid, shoulder_curve_amount)
    )
    abs(shoulder_curve_amount) <= 0.001
        ? target
        : iterations <= 0
            ? mid
            : mid_z < target
                ? keycap_shell_shoulder_plan_fraction_for_z(target, shoulder_curve_amount, iterations - 1, mid, high)
                : keycap_shell_shoulder_plan_fraction_for_z(target, shoulder_curve_amount, iterations - 1, low, mid);
...
function keycap_dish_max_rise(dish_type, dish_depth) =
    keycap_dish_is_active(dish_type, dish_depth) && dish_depth < 0
        ? -dish_depth
        : 0;
...
function keycap_clamp_dish_depth(
    ...
    let(
        depth_limit = keycap_dish_depth_limit(
            dish_type,
            dish_radius,
            top_left,
            top_right,
    ...
    min(max(dish_depth, -depth_limit), depth_limit);
...
// A convex dish needs the side draft continued above the flat top plane;
// a vertical footprint clip would leave a raised lip around the keytop.
module keycap_top_tapered_prism(
    left,
    right,
    front,
    back,
    ...
    join_overlap = 0.02;
    safe_left_slope = max(left_slope, 0);
    safe_right_slope = max(right_slope, 0);
    safe_front_slope = max(front_slope, 0);
    safe_back_slope = max(back_slope, 0);
    requested_left_inset = safe_height * safe_left_slope;
    requested_right_inset = safe_height * safe_right_slope;
    requested_front_inset = safe_height * safe_front_slope;
    requested_back_inset = safe_height * safe_back_slope;
    inset_scale = min(
        1,
        requested_left_inset + requested_right_inset <= 0.001
            ? 1
            : max((right - left - 0.1) / (requested_left_inset + requested_right_inset), 0),
        requested_front_inset + requested_back_inset <= 0.001
            ? 1
            : max((back - front - 0.1) / (requested_front_inset + requested_back_inset), 0)
    );
    effective_left_slope = safe_left_slope * inset_scale;
    effective_right_slope = safe_right_slope * inset_scale;
    effective_front_slope = safe_front_slope * inset_scale;
    effective_back_slope = safe_back_slope * inset_scale;
    left_inset = safe_height * effective_left_slope;
    right_inset = safe_height * effective_right_slope;
    front_inset = safe_height * effective_front_slope;
    back_inset = safe_height * effective_back_slope;
    low_left = left - join_overlap * effective_left_slope;
    low_right = right + join_overlap * effective_right_slope;
    low_front = front - join_overlap * effective_front_slope;
    low_back = back + join_overlap * effective_back_slope;
    high_left = left + left_inset;
    high_right = right - right_inset;
    high_front = front + front_inset;
    high_back = back - back_inset;
    low_radius = max(radius + join_overlap * min(
        effective_left_slope,
        effective_right_slope,
        effective_front_slope,
        effective_back_slope
    ), 0);
    high_radius = max(radius - min(left_inset, right_inset, front_inset, back_inset), 0);
    low_corner_radii = keycap_corner_radii_add(corner_radii, low_radius - radius);
    high_corner_radii = keycap_corner_radii_add(corner_radii, high_radius - radius);
    has_taper = max(
        effective_left_slope,
        effective_right_slope,
        effective_front_slope,
    ...
    keycap_top_plane_transform(top_center_height, pitch_deg, roll_deg, top_offset_x, top_offset_y)
        if (!has_taper) {
            translate([0, 0, base_z - join_overlap])
                linear_extrude(height = safe_height + join_overlap + 0.01)
                    if (is_undef(low_corner_radii)) {
                        rounded_rect_coords(
                            low_left,
                            low_right,
                            low_front,
            ...
            hull() {
                ...
                translate([0, 0, base_z + safe_height])
                    linear_extrude(height = 0.01)
                        if (is_undef(high_corner_radii)) {
                            rounded_rect_coords(
                                high_left,
                                high_right,
                                high_front,
...
module keycap_rect_inner_clearance_volume(
    left,
    right,
    front,
    back,
    ...
    if (right - left > 0.001 && back - front > 0.001) {
        ...
        side_left_inset = top_center_height * tan(left_angle);
        side_right_inset = top_center_height * tan(right_angle);
        side_front_inset = top_center_height * tan(front_angle);
        side_back_inset = top_center_height * tan(back_angle);
        shoulder_outset = max(min(
            max(side_left_inset, 0),
            max(side_right_inset, 0),
            max(side_front_inset, 0),
            max(side_back_inset, 0)
        ), 0);
        shoulder_curve_amount = keycap_shell_shoulder_curve_amount(
            top_center_height,
            shoulder_outset,
            shoulder_radius
        );
        shoulder_steps = keycap_top_hat_shoulder_curve_steps(abs(shoulder_radius), quality);
        inner_z_fraction = top_center_height <= 0.001
            ? 1
            : min(max(inner_height / top_center_height, 0), 1);
        top_plan_fraction = keycap_shell_shoulder_plan_fraction_for_z(inner_z_fraction, shoulder_curve_amount);
        section_steps = max(ceil(shoulder_steps * max(top_plan_fraction, 0.001)), 1);
        ...
        base_corner_radii = is_undef(top_corner_radii)
            ? undef
            : keycap_corner_radius_list(bottom_corner_radius);
        ...
        if (abs(shoulder_curve_amount) <= 0.001) {
            ...
            union() {
                ...
                for (step = [0 : section_steps - 1]) {
                    hull() {
                        for (j = [step : step + 1]) {
                            local_t = j / section_steps;
                            plan_fraction = top_plan_fraction * local_t;
                            z_fraction = keycap_shell_shoulder_z_fraction(plan_fraction, shoulder_curve_amount);
                            keycap_shell_section_face(
                                left = left + side_left_inset * plan_fraction,
                                right = right - side_right_inset * plan_fraction,
                                front = front + side_front_inset * plan_fraction,
                                back = back - side_back_inset * plan_fraction,
                                radius = bottom_corner_radius + (top_corner_radius - bottom_corner_radius) * local_t,
                                z = top_center_height * z_fraction,
                                pitch_deg = pitch_deg,
                                roll_deg = roll_deg,
                                quality = quality,
                                top_offset_x = top_offset_x * local_t,
                                top_offset_y = top_offset_y * local_t,
                                tilt_amount = local_t,
                                corner_radii = keycap_corner_radii_lerp(base_corner_radii, top_corner_radii, local_t)
                            );
                        }
                    }
                }
...
module keycap_inner_clearance_volume(
    ...
    keycap_rect_inner_clearance_volume(
        ...
        bottom_corner_radius = keycap_inner_corner_radius(bottom_corner_radius, wall),
        top_corner_radius = keycap_inner_corner_radius(top_corner_radius, wall),
        pitch_deg = pitch_deg,
        roll_deg = roll_deg,
        quality = quality,
        top_offset_x = top_offset_x,
        ...
        top_corner_radii = inner_top_corner_radii,
...
module keycap_shell(
    ...
    top_surface_inset = keycap_top_edge_surface_inset(
        ...
        top_center_height,
        shoulder_outset,
        max(top_right - top_left, 0.2),
        max(top_back - top_front, 0.2)
    );
    ...
    surface_top_corner_radii = keycap_corner_radii_add(
        resolved_top_corner_radii,
        -top_surface_inset
    );
    ...
    difference() {
        union() {
            keycap_apply_top_surface(
                ...
                // Keep the positive dish footprint as the signed curve reference.
                ...
                top_offset_x = top_offset_x,
                top_offset_y = top_offset_y,
                top_corner_radii = surface_top_corner_radii,
